...
| Операционная система | RAW 2.4.24.0<br>Web Media 1.2.29.0 | RAW 2.5.x.x<br>Web Media 2.x.x.x | JPEG XL |
|---|:---:|:---:|:---:|
| Windows 10 (билд < 22000) | ✅ | ❌ | ❌ |
| Windows 11 (билд 22000–26099, до 24H2) | ✅ | ✅ | ❌ |
| Windows 11 24H2+ (билд ≥ 26100) | ✅ | ✅ | ✅ |

> [!WARNING]
> Актуальные версии "RAW Image Extension" (начиная с 2.4.9.1) на Windows 10 не устанавливаются — Add-AppxPackage завершается ошибкой `0x80073CFD` ("не удалось выполнить необходимое условие для установки"), так как пакет требует ОС версии 10.0.22621.0 (Windows 11 22H2) и выше. Поэтому на всех системах ставится закреплённая версия `2.4.24.0`, а более новая — только на Windows 11.
>
> Актуальные версии "Web Media Extensions" (2.x) на Windows 10 не устанавливаются — Add-AppxPackage завершается ошибкой валидации манифеста (`0xC00CE015`), так как манифест использует более новую схему, которую понимает только Windows 11. Поэтому на всех системах ставится закреплённая версия `1.2.29.0`, а более новая версия дополнительно устанавливается только на Windows 11.
>
> Кодек "JPEG XL Image Extension" на Windows 10 не устанавливается — Add-AppxPackage завершается ошибкой `0x80073CFD` ("не удалось выполнить необходимое условие для установки"), так как пакет требует ОС версии 10.0.26100.0 (Windows 11 24H2) и выше. Официальный кодек от Microsoft жёстко завязан на инфраструктуру Windows 11: системный WIC (Windows Imaging Component) в Windows 10 не обновлялся под поддержку JPEG XL на уровне API Проводника, поэтому Win10-варианта попросту не существует.
...
├── Install.bat
└── Extensions/
    ├── AV1_Video_Extension_2.0.30.0.AppxBundle
    ├── HEIF_Image_Extension_1.2.48.0.AppxBundle
    ├── HEVC_Video_Extension_2.4.110.0.AppxBundle
