...
for target in TARGETS:
    ...
    if not html:
        ...
        notes.append(f"🔴 `{nice_name}` — Не обновилось")
        continue
    ...
    if candidates:
        ...
        pin_config = PINNED_PACKAGES.get(nice_name)
        pinned_prefix = f"{nice_name}_{pin_config['pinned_version']}" if pin_config else None
        ...
        if existing_files:
            search_files = (
                [f for f in existing_files if pinned_prefix not in os.path.basename(f)]
                if pin_config
                else existing_files
            )
            if search_files:
                old_file = os.path.basename(search_files[0])
                old_ver_match = re.search(r'_(\d+\.\d+\.\d+\.\d+)\.', old_file)
                if old_ver_match:
                    old_version = old_ver_match.group(1)

        if pin_config:
            if best['version_tuple'] < pin_config['min_update_version']:
                print(f"{nice_name}: найденная версия {best_version} ниже минимальной для обновления — пропускаем.")
                notes.append(f"🔴 `{nice_name}_{best_version}` — Ниже минимальной версии для Win11, пропущено")
                continue

            existing_updatable = [
                f for f in existing_files
                if pinned_prefix not in os.path.basename(f)
            ]

            if old_version and old_version == best_version:
                ...
                if old_version and best['version_tuple'] < parse_version(old_version):
                    print(f"На сервере версия {best_version} ниже установленной {old_version} — откат.")
                    notes.append(f"🟡 `{nice_name}_{old_version}` — Откатилось до `{best_version}`")
                elif old_version:
                    print(f"Найдено обновление {nice_name} (Win11): {old_version} -> {best_version}")
                    notes.append(f"🟢 `{nice_name}_{old_version}` — Обновилось до `{best_version}`")
                else:
                    print(f"Новый пакет {nice_name} (Win11): {best_version}")
                    notes.append(f"🔵 `{nice_name}_{best_version}` — Добавлено")

                for ef in existing_updatable:
                    os.remove(ef)
        ...
        notes.append(f"⚠️ `{nice_name}` — Не найдено подходящих файлов на сервере")
...
for name_, cfg in PINNED_PACKAGES.items():
    pinned_name = f"{name_}_{cfg['pinned_version']}"
    for f in sorted(glob.glob(os.path.join(EXT_DIR, f"{pinned_name}.*"))):
        name = os.path.basename(f)
        if name not in actual_files:
            actual_files.append(name)
